...
#
CHAMPION_DATA: List[Tuple[str, int, Tuple[str, ...]]] = [  # pragma: no cover
...
# ============================================================
TRAIT_BREAKPOINTS: Dict[str, List[int]] = {  # pragma: no cover
...
all_traits_set: set[str] = set()
for _name, _cost, ts in CHAMPION_DATA:
    for t in ts:
...
def combine_and_search(
    left_sigs: List[Signature], right_sigs: List[Signature], start_time: float
) -> List[BestTeam]:
...
def main() -> None:
    ...
    best_teams = combine_and_search(left_sigs, right_sigs, start_global)
    ...
    print(
        "\nHinweis: Passe compute_score() und LEVEL_WEIGHT / Parameter an dein Original an, damit die Score-Werte exakt den bisherigen entsprechen."
    )
